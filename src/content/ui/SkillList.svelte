...
<div class="dsm-section-title">
  <div style="display: flex; align-items: center; justify-content: space-between; width: 100%;">
    ...
    <div style="display: flex; gap: 6px;">
      <button type="button" class="dsm-btn-outlined" onclick={exportSkills}>
        {t('skillList.export')}
      </button>
      <button type="button" class="dsm-btn-outlined" onclick={triggerImport}>
        {t('skillList.import')}
      </button>
    </div>
  </div>
...
<div style="margin-top: 16px;">
  <label class="dsm-label" for="dsm-skill-upload">{t('skillList.uploadLabel')}</label>
  <input
    id="dsm-skill-upload"
    type="file"
    accept=".md,.json"
...
<div style="margin-top: 12px;">
  ...
  <label class="dsm-label">{t('skillList.urlLabel')}</label>
...
<div id="dsm-skill-list" class="dsm-list">
  {#if skills.length === 0}
    <p class="dsm-empty">{t('skillList.empty')}</p>
  {:else}
    {#each skills as skill (skill.id)}
      {#if editingId === skill.id}
        <div class="dsm-inline-editor">
          <input 
            class="dsm-input" 
          ...
          ></textarea>
          <div class="dsm-editor-actions">
            <button type="button" class="dsm-btn-outlined" onclick={cancelEdit}>{t('skillList.cancel')}</button>
            <button type="button" class="dsm-btn" onclick={saveEdit}>{t('skillList.save')}</button>
          </div>
        </div>
      {:else}
        <div class="dsm-skill-item">
          <label>
            <input
              type="checkbox"
          ...
          <div style="display: flex; gap: 6px;">
            <button type="button" class="dsm-btn-outlined" style="font-size: 11px; padding: 4px 8px;" onclick={() => startEdit(skill)}>
              {t('skillList.edit')}
            </button>
            <button type="button" class="dsm-btn-danger" onclick={() => deleteSkill(skill.id)}>
              {t('skillList.delete')}
            </button>
          </div>
        </div>
