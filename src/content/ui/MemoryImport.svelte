...
<div class="dsm-mi-backdrop" onclick={onclose} role="presentation">
  <div
    bind:this={modalRef}
    class="dsm-mi-modal"
    ...
    tabindex="-1"
  >
    <div class="dsm-mi-header">
      <div class="dsm-mi-header-left">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/>
          <polyline points="7 10 12 15 17 10"/>
          <line x1="12" y1="15" x2="12" y2="3"/>
        </svg>
        <span class="dsm-mi-title">{t('memoryImport.title')}</span>
      </div>
      <button class="dsm-mi-close-btn" onclick={onclose} aria-label="Close">
    ...
    <div class="dsm-mi-body">
      <div class="dsm-mi-step">
        <div class="dsm-mi-step-label">{t('memoryImport.step1Title')}</div>
        <div class="dsm-mi-prompt-box">
          <textarea class="dsm-mi-prompt-textarea" readonly value={MEMORY_EXPORT_PROMPT}></textarea>
          <button type="button" class="dsm-mi-copy-btn" onclick={copyPrompt}>
            {#if copied}
        ...
        <div class="dsm-mi-ai-grid">
          {#each AI_BUTTONS as btn}
            <button
              type="button"
              class="dsm-mi-ai-btn"
              style="--dsm-ai-color: {btn.color}"
              onclick={() => openAi(btn.url)}
            >
              <span class="dsm-mi-ai-icon">{@html btn.icon}</span>
              <span class="dsm-mi-ai-label">{btn.name}</span>
            </button>
          {/each}
        </div>
      </div>
      ...
      <div class="dsm-mi-step">
        <div class="dsm-mi-step-label">{t('memoryImport.step2Title')}</div>
        <textarea
          class="dsm-mi-textarea"
        ...
        {#if importStatus === "error"}
          <p class="dsm-mi-error">{t('memoryImport.importError')}</p>
        {/if}
      </div>
    </div>
  </div>
