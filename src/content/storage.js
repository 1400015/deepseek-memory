...
export async function loadStateFromStorage() {
  ...
  const localeValues = await chrome.storage.local.get(["dsm_locale_updates"]);
  if (localeValues.dsm_locale_updates) {
    i18n.loadUpdatedLocales(localeValues.dsm_locale_updates);
  }
...
export function bindStorageChangeListener() {
  chrome.storage.onChanged.addListener((changes, areaName) => {
    ...
    if (changes.dsm_locale_updates) {
      if (changes.dsm_locale_updates.newValue) {
        i18n.loadUpdatedLocales(changes.dsm_locale_updates.newValue);
      } else {
        i18n.resetLocales();
      }
    }
