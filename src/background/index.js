...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message.type === "DSM_UPDATE_LANGUAGES") {
    fetch("https://raw.githubusercontent.com/mwahidops/deepseek-memory/main/static/remote-locales.json")
      ...
      .then((updates) => {
        ...
        chrome.storage.local.set({ dsm_locale_updates: updates || {} });
        chrome.storage.local.set({ dsm_locale_update_last_checked: new Date().toLocaleDateString() });
  ...
  if (message.type === "DSM_RESET_LANGUAGES") {
    chrome.storage.local.remove("dsm_locale_updates");
    sendResponse({ success: true });
    return true;
  }
  ...
  if (message.type === "DSM_FETCH_SKILL_URL") {
