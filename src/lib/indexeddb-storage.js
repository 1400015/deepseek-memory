...
export async function migrateFromChromeStorage() {
  try {
    ...
    const result = await new Promise((resolve) => {
      chrome.storage.local.get(["dsm_memories", "dsm_settings", "dsm_skills"], resolve);
    });

    // Migrate memories
    if (result.dsm_memories && typeof result.dsm_memories === "object") {
      const writes = Object.entries(result.dsm_memories).map(([key, item]) => ({
        key,
        value: item.value,
    ...
    // Migrate settings
    if (result.dsm_settings) {
      await saveSettings(result.dsm_settings);
    }

    // Migrate skills
    if (Array.isArray(result.dsm_skills)) {
      await saveSkills(result.dsm_skills);
    }
